...
4. 참고 사이트에서 무엇을 가져오고 피할지 `reference-dna.md`로 정리합니다.
5. 다시 쓸 수 있는 지시문을 `prompt-pack.md`로 저장합니다.
6. AI가 이해할 수 있는 디자인 문서인 `DESIGN.md`를 만듭니다.
7. Codex에게 그 문서를 넘겨 홈페이지를 만듭니다.
8. 마우스 반응, 스크롤 움직임, 모바일 화면, 링크 공유 설정을 확인합니다.
9. GitHub와 Vercel을 이용해 실제 링크로 배포합니다.

## 이번 버전에서 좋아진 점

이 에이전트는 [MengTo/Skills](https://github.com/MengTo/Skills)의 좋은 접근을 참고해 업그레이드했습니다.

핵심은 간단합니다.

좋은 홈페이지는 “예쁘게 만들어줘”로 나오지 않습니다. 마음에 드는 사이트를 보고, 어떤 화면을 봤는지, 무엇을 가져올지, 무엇은 피할지, 어떤 지시문으로 다시 쓸지를 남겨야 합니다.

그래서 이 버전은 `reference-dna.md`와 `prompt-pack.md`를 더 중요하게 다룹니다.

## 현재 에이전트의 스킬 목록과 흐름

이 에이전트의 중심 스킬은 하나입니다.

```text
personal-brand-site
```

이 스킬 안에서 아래 단계들이 차례대로 실행됩니다.
...
이번 버전부터는 MengTo/Skills의 핵심 기술 스킬 8개를 내부에 함께 넣었습니다.

| 내부 스킬 | 쉽게 말하면 | 언제 쓰나요 |
| --- | --- | --- |
| `stitched-full-page-capture` | 사이트 전체를 나눠서 캡처하는 방법 | 마음에 드는 레퍼런스 사이트를 정확히 분석할 때 |
| `html-to-interaction-prompts` | 움직임을 프롬프트로 바꾸는 방법 | 마우스 반응, 클릭 반응, 스크롤 효과를 따라 만들고 싶을 때 |
| `design-first-ui-prompting` | AI가 이해하는 디자인 지시문 만들기 | `DESIGN.md`를 만들기 전에 감각을 구체화할 때 |
| `high-end-visual-design` | 흔한 AI 템플릿처럼 보이지 않게 막는 규칙 | 결과물이 평범하거나 싸 보일 때 |
| `landing-page` | 한 사람에게 한 행동을 시키는 페이지 구조 | 상담, 신청, 구매, 자료 요청 같은 목적을 선명하게 할 때 |
| `animation-on-scroll` | 스크롤할 때 자연스럽게 나타나는 효과 | 초보자도 수정 가능한 기본 움직임을 넣을 때 |
| `cinematic-scroll-storytelling` | 내려가면서 이야기가 펼쳐지는 홈페이지 | 더 감각적인 스크롤텔링을 만들 때 |
| `optimize-web-animations` | 움직임이 버벅이지 않게 확인하는 법 | 애니메이션이 많아진 뒤 모바일/성능을 확인할 때 |

원본 스킬 파일은 아래 폴더에 있습니다.

```text
skills/personal-brand-site/vendor/mengto-skills/
```

MIT 라이선스 고지도 함께 포함했습니다.
...
이 파일에는 단순히 주소만 적지 않습니다.

- 어떤 화면을 봤는지
- 어떤 부분이 마음에 들었는지
- 무엇을 가져올지
- 무엇은 절대 따라 하지 않을지
- AI에게 줄 자료가 URL인지, 스크린샷인지, 색상표인지, 움직임 설명인지

이렇게 적어야 Codex가 “분위기만 비슷한 흔한 페이지”로 도망가지 않습니다.

### `prompt-pack.md`

홈페이지 제작에 계속 다시 쓸 수 있는 지시문 모음입니다.
...
- 스크롤 움직임을 만드는 지시문
- 이미지를 생성하는 지시문
- 마음에 안 들 때 수정 요청하는 지시문
- 하면 안 되는 디자인을 막는 지시문

쉽게 말하면 `prompt-pack.md`는 **내 홈페이지를 계속 고쳐가기 위한 AI 지시문 창고**입니다.
...
이 명령어는 예제 `DESIGN.md`가 홈페이지 제작에 필요한 내용을 갖추고 있는지 확인합니다. 이제는 레퍼런스 증거, 디자인 시스템 제약, 프롬프트팩 요약까지 들어있는지도 같이 봅니다.
...
먼저 나를 정리하고, 참고할 감각을 고르고, 레퍼런스 증거와 프롬프트팩을 만든 뒤, `DESIGN.md`를 기준으로 홈페이지를 제작합니다.
